...
public class GetAllPhotosForFront : EndpointBaseAsync
    ...
    .WithActionResult
{
    ...
    public GetAllPhotosForFront(IPhotoService<Photo> service, IMapper mapper)
    {
        _service = service;
        _mapper = mapper;
    }

    [HttpGet("clubs/photos")]
    [SwaggerOperation(
    ...
    public override async Task<ActionResult> HandleAsync(Pagination pagination, CancellationToken token = default)
    {
        ...
        var map = _mapper.Map<List<GetAllPhotosForFrontResult>>(result);
        return Ok(map);
    }
}